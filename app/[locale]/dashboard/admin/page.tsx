import {
  ...
  UserCheck, AlertCircle, GraduationCap, CalendarDays, ChevronRight, ArrowUpRight,
} from 'lucide-react';
...
const QUICK_STATS = [
  { label: 'Moy. générale', value: '14.8/20', color: '#2563EB' },
  { label: 'Présence',       value: '91%',     color: '#16A34A' },
  { label: 'Questions / j', value: '8.4',      color: '#7C3AED' },
];
    ...
    <div className="p-4 md:p-6 max-w-6xl space-y-6">
      ...
      >
        <div className="relative z-10">
          <p className="text-white/70 text-sm mb-1">Tableau de bord admin</p>
          <h1 className="font-display font-black text-2xl md:text-3xl">PES Douala</h1>
          <p className="text-white/60 text-sm mt-1">Année scolaire 2025–2026 · Trimestre 2</p>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-4">
        {KPI_CARDS.map(({ label, value, delta, icon: Icon, color, bg }) => (
          <div
            key={label}
            className="p-4 md:p-5 rounded-2xl border border-[var(--border-color)] bg-[var(--background)] hover:shadow-[0_4px_20px_rgb(0_0_0/0.08)] transition-all group cursor-pointer"
          >
            <div className="flex items-start justify-between mb-4">
              <div className="w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0" style={{ backgroundColor: bg }}>
                <Icon size={20} style={{ color }} />
              </div>
              <ArrowUpRight size={15} className="text-[var(--text-muted)] opacity-0 group-hover:opacity-100 transition-opacity" />
            </div>
            <p className="font-display font-black text-3xl text-[var(--text-primary)] leading-none">{value}</p>
            <p className="text-sm text-[var(--text-secondary)] mt-1 mb-2">{label}</p>
      ...
      <div className="rounded-2xl border border-[var(--border-color)] bg-[var(--background)] overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-[var(--border-color)]">
          ...
          <Link href="users" className="text-sm text-[var(--accent-primary)] font-medium hover:underline flex items-center gap-1">
        ...
        <div className="divide-y divide-[var(--border-color)]">
          {RECENT_INSCRIPTIONS.map(({ name, role, detail, status, time, avatar, color }) => {
            ...
            return (
              <div key={name} className="flex items-center gap-4 px-5 py-4 hover:bg-[var(--background-soft)] transition-colors">
                <div
                ...
                <div className="flex flex-col items-end gap-1.5">
                  <span className="text-xs font-semibold px-2.5 py-1 rounded-lg" style={{ backgroundColor: st.bg, color: st.color }}>
                    {st.label}
                  </span>
